/**
 * Script de Migração - Pesca Profissional 2023
 *
 * Planilha: Pesca Profissional - 2023.xlsx
 * Total estimado: 6 registros
 * Programa no SIGMA: ID 43 - "Pescador Profissional (Legado)"
 *
...
const ARQUIVO_PLANILHA = 'C:\\Users\\marce\\Downloads\\2023\\Pesca Profissional - 2023.xlsx';
const PROGRAMA_ID = 43; // Pescador Profissional (Legado)
const ANO_REFERENCIA = 2023;

interface RegistroPescaProfissional {
  numero: number;
  produtor: string;
  linha: string;
  quantidade: number;
...
function extrairDadosAba(workbook: XLSX.WorkBook, sheetName: string): RegistroPescaProfissional[] {
  const sheet = workbook.Sheets[sheetName];
  const dados = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: '', raw: true }) as any[][];

  const registros: RegistroPescaProfissional[] = [];

  // Encontrar linha do cabeçalho
  let headerIndex = -1;
  for (let i = 0; i < Math.min(10, dados.length); i++) {
    const row = dados[i];
    if (row && row.some(cell => String(cell).includes('PRODUTOR') || String(cell).includes('PESCADOR'))) {
      headerIndex = i;
      break;
    }
  }
  ...
  const idxProdutor = header.findIndex(h => h.includes('PRODUTOR') || h.includes('PESCADOR'));
...
async function migrarPescaProfissional() {
  console.log('='.repeat(80));
  console.log('MIGRAÇÃO - PESCA PROFISSIONAL 2023');
  console.log('='.repeat(80));
  ...
  const todosRegistros: RegistroPescaProfissional[] = [];
