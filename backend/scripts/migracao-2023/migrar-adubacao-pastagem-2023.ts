/**
 * Script de Migração - Adubação de Pastagem 2023
 *
 * Planilha: Adubação Pastagem - 2023.xlsx
 * Total estimado: 19 registros
 * Programa no SIGMA: ID 44 - "Adubação de Pastagem (Legado)"
 *
...
const ARQUIVO_PLANILHA = 'C:\\Users\\marce\\Downloads\\2023\\Adubação Pastagem - 2023.xlsx';
const PROGRAMA_ID = 44; // Adubação de Pastagem (Legado)
const ANO_REFERENCIA = 2023;

interface RegistroAdubacaoPastagem {
  numero: number;
  produtor: string;
  linha: string;
  quantidade: number;
...
function extrairDadosAba(workbook: XLSX.WorkBook, sheetName: string): RegistroAdubacaoPastagem[] {
  const sheet = workbook.Sheets[sheetName];
  const dados = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: '', raw: true }) as any[][];

  const registros: RegistroAdubacaoPastagem[] = [];
...
async function migrarAdubacaoPastagem() {
  console.log('='.repeat(80));
  console.log('MIGRAÇÃO - ADUBAÇÃO DE PASTAGEM 2023');
  console.log('='.repeat(80));
  ...
  const todosRegistros: RegistroAdubacaoPastagem[] = [];
  ...
  console.log(`Quantidade total: ${stats.quantidadeTotal.toFixed(2)}`);
