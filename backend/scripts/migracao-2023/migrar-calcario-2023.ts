/**
 * Script de Migração - Calcário 2023
 *
 * Planilha: Calcário - 2023.xlsx
 * Total estimado: 188 registros
 * Programa no SIGMA: ID 64 - "Correção de Solos - PRÓSOLOS"
 *
...
const ARQUIVO_PLANILHA = 'C:\\Users\\marce\\Downloads\\2023\\Calcário - 2023.xlsx';
const PROGRAMA_ID = 64; // Correção de Solos - PRÓSOLOS
const ANO_REFERENCIA = 2023;

interface RegistroCalcario {
  numero: number;
  produtor: string;
  linha: string;
  toneladas: number;
...
function extrairDadosAba(workbook: XLSX.WorkBook, sheetName: string): RegistroCalcario[] {
  const sheet = workbook.Sheets[sheetName];
  const dados = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: '', raw: true }) as any[][];

  const registros: RegistroCalcario[] = [];
...
async function migrarCalcario() {
  console.log('='.repeat(80));
  console.log('MIGRAÇÃO - CALCÁRIO 2023');
  console.log('='.repeat(80));
  ...
  const todosRegistros: RegistroCalcario[] = [];
