/**
 * Script de Migração - Aveia 2023
 *
 * Planilha: Aveia - 2023.xlsx
 * Total estimado: 97 registros
 * Programa no SIGMA: ID 66 - "Cobertura do Solo - Aveia"
 *
 * Estrutura da planilha:
 * - Colunas: Nº | PRODUTOR | LINHA | Quant. (Kg) | Nº AUTOR. | DATA | R$
 * - Abas: Abril 2023, Maio 2023, ..., Outubro 2023
 */
...
const ARQUIVO_PLANILHA = 'C:\\Users\\marce\\Downloads\\2023\\Aveia - 2023.xlsx';
const PROGRAMA_ID = 66; // Cobertura do Solo - Aveia
const ANO_REFERENCIA = 2023;

interface RegistroAveia {
  numero: number;
  produtor: string;
  linha: string;
  quantidadeKg: number;
...
function extrairDadosAba(workbook: XLSX.WorkBook, sheetName: string): RegistroAveia[] {
  const sheet = workbook.Sheets[sheetName];
  const dados = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: '', raw: true }) as any[][];

  const registros: RegistroAveia[] = [];
  ...
  const idxQuant = header.findIndex(h => h.includes('QUANT') || h.includes('KG'));
  const idxAutor = header.findIndex(h => h.includes('AUTOR') || h.includes('AUT'));
  ...
  for (let i = headerIndex + 1; i < dados.length; i++) {
    ...
    registros.push({
      ...
      quantidadeKg: Number(row[idxQuant]) || 0,
      numeroAutorizacao: String(row[idxAutor] || ''),
      data,
...
async function migrarAveia() {
  console.log('='.repeat(80));
  console.log('MIGRAÇÃO - AVEIA 2023');
  console.log('='.repeat(80));
  ...
  const todosRegistros: RegistroAveia[] = [];
    ...
    try {
      ...
      await prisma.solicitacaoBeneficio.create({
        data: {
          ...
          datasolicitacao: registro.data,
          status: 'concluido',
          observacoes: `Migrado da planilha 2023 | Aut: ${registro.numeroAutorizacao} | Linha: ${registro.linha}`,
          quantidadeSolicitada: registro.quantidadeKg,
      ...
      stats.quantidadeTotal += registro.quantidadeKg;
  ...
  console.log(`Quantidade total: ${stats.quantidadeTotal.toFixed(2)} Kg`);
