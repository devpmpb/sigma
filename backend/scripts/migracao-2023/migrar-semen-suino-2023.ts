/**
 * Script de Migração - Sêmen Suíno 2023
 *
 * Planilha: Sêmen Suíno 2023.xlsx
 * Total estimado: 35 registros
 * Programa no SIGMA: ID 72 - "Melhoria Genética Suínos"
 *
 * Estrutura da planilha:
 * - Colunas: Nº | PRODUTOR | LINHA | Quant. | Nº Autor | DATA
 * - Abas: meses de 2023
 */
...
const ARQUIVO_PLANILHA = 'C:\\Users\\marce\\Downloads\\2023\\Sêmen Suíno 2023.xlsx';
const PROGRAMA_ID = 72; // Melhoria Genética Suínos
const ANO_REFERENCIA = 2023;

interface RegistroSemenSuino {
  numero: number;
  produtor: string;
  linha: string;
  quantidade: number;
...
function extrairDadosAba(workbook: XLSX.WorkBook, sheetName: string): RegistroSemenSuino[] {
  const sheet = workbook.Sheets[sheetName];
  const dados = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: '', raw: true }) as any[][];

  const registros: RegistroSemenSuino[] = [];
...
async function migrarSemenSuino() {
  console.log('='.repeat(80));
  console.log('MIGRAÇÃO - SÊMEN SUÍNO 2023');
  console.log('='.repeat(80));
  ...
  const todosRegistros: RegistroSemenSuino[] = [];
