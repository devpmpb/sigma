/**
 * Script de Migração - Cama de Aviário 2023
 *
 * Planilha: Cama de Aviário - 2023.xlsx
 * Total estimado: 57 registros
 * Programa no SIGMA: ID 65 - "Adubo Orgânico Sólido (Pró-Orgânico)"
 *
 * Estrutura da planilha:
 * - Colunas: Nº | PRODUTOR | LINHA | Ton | DATA | R$
 * - Abas: meses de 2023
 */
...
const ARQUIVO_PLANILHA = 'C:\\Users\\marce\\Downloads\\2023\\Cama de Aviário - 2023.xlsx';
const PROGRAMA_ID = 65; // Adubo Orgânico Sólido (Pró-Orgânico)
const ANO_REFERENCIA = 2023;

interface RegistroCamaAviario {
  numero: number;
  produtor: string;
  linha: string;
  toneladas: number;
...
function extrairDadosAba(workbook: XLSX.WorkBook, sheetName: string): RegistroCamaAviario[] {
  const sheet = workbook.Sheets[sheetName];
  const dados = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: '', raw: true }) as any[][];

  const registros: RegistroCamaAviario[] = [];
  ...
  const idxTon = header.findIndex(h => h.includes('TON'));
...
async function migrarCamaAviario() {
  console.log('='.repeat(80));
  console.log('MIGRAÇÃO - CAMA DE AVIÁRIO 2023');
  console.log('='.repeat(80));
  ...
  const todosRegistros: RegistroCamaAviario[] = [];
    ...
    try {
      ...
      const existente = await prisma.solicitacaoBeneficio.findFirst({
        where: {
          ...
          quantidadeSolicitada: registro.toneladas
        }
      });
      ...
      stats.quantidadeTotal += registro.toneladas;
  ...
  console.log(`Quantidade total: ${stats.quantidadeTotal.toFixed(2)} Toneladas`);
