/**
 * Script de Migração - Esterco Líquido 2023
 *
 * Planilha: Esterco Líquido - 2023.xlsx
 * Total estimado: 143 registros
 * Programas no SIGMA:
 *   - ID 56: "Esterco Líquido - Bovinocultura de leite - 2023"
 *   - ID 57: "Esterco Líquido - Produção Agrícola - 2023"
 *
 * Estrutura da planilha:
 * - Colunas: Nº | PRODUTOR | LINHA | Quant. | DATA | R$
 * - Abas: Fevereiro 2023, Março 2023, ..., Novembro 2023
 */
...
const ARQUIVO_PLANILHA = 'C:\\Users\\marce\\Downloads\\2023\\Esterco Líquido - 2023.xlsx';
// Por padrão usar Bovinocultura, pode ser ajustado manualmente depois
const PROGRAMA_ID = 56; // Esterco Líquido - Bovinocultura de leite - 2023
const ANO_REFERENCIA = 2023;

interface RegistroEsterco {
  numero: number;
  produtor: string;
  linha: string;
  quantidade: number;
...
function extrairDadosAba(workbook: XLSX.WorkBook, sheetName: string): RegistroEsterco[] {
  const sheet = workbook.Sheets[sheetName];
  const dados = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: '', raw: true }) as any[][];

  const registros: RegistroEsterco[] = [];
  ...
  const idxQuant = header.findIndex(h => h.includes('QUANT') || h.includes('CARGAS') || h.includes('M³'));
...
async function migrarEstercoLiquido() {
  console.log('='.repeat(80));
  console.log('MIGRAÇÃO - ESTERCO LÍQUIDO 2023');
  console.log('='.repeat(80));
  ...
  const todosRegistros: RegistroEsterco[] = [];
