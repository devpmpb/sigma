/**
 * Script de Migração - Sêmen Bovino 2023
 *
 * Planilha: Sêmen Bovino 2023.xlsx
 * Total estimado: 217 registros
 * Programa no SIGMA: ID 47 - "Sêmen Bovino" ou ID 71 - "Sêmen Sexado"
 *
 * Estrutura da planilha:
 * - Colunas: Nº | PRODUTOR | LINHA | Quant. | Nº Autor | Touro | DATA
 * - Abas: meses de 2023
 */
...
const ARQUIVO_PLANILHA = 'C:\\Users\\marce\\Downloads\\2023\\Sêmen Bovino 2023.xlsx';
const PROGRAMA_ID = 47; // Sêmen Bovino (usar 71 se for sexado)
const ANO_REFERENCIA = 2023;

interface RegistroSemenBovino {
  numero: number;
  produtor: string;
  linha: string;
  quantidade: number;
...
function extrairDadosAba(workbook: XLSX.WorkBook, sheetName: string): RegistroSemenBovino[] {
  const sheet = workbook.Sheets[sheetName];
  const dados = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: '', raw: true }) as any[][];

  const registros: RegistroSemenBovino[] = [];
  ...
  const idxTouro = header.findIndex(h => h.includes('TOURO'));
  ...
  for (let i = headerIndex + 1; i < dados.length; i++) {
    ...
    registros.push({
      ...
      quantidade: Number(row[idxQuant]) || 1,
      numeroAutorizacao: String(row[idxAutor] || ''),
      touro: String(row[idxTouro] || ''),
      data,
      mes: sheetName
    });
  }
...
async function migrarSemenBovino() {
  console.log('='.repeat(80));
  console.log('MIGRAÇÃO - SÊMEN BOVINO 2023');
  console.log('='.repeat(80));
  ...
  const todosRegistros: RegistroSemenBovino[] = [];
    ...
    try {
      ...
      await prisma.solicitacaoBeneficio.create({
        data: {
          ...
          datasolicitacao: registro.data,
          status: 'concluido',
          observacoes: `Migrado da planilha 2023 | Aut: ${registro.numeroAutorizacao} | Touro: ${registro.touro} | Linha: ${registro.linha}`,
  ...
  console.log(`Quantidade total: ${stats.quantidadeTotal} doses`);
