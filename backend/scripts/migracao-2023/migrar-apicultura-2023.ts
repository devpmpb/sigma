/**
 * Script de Migração - Apicultura 2023
 *
 * Planilha: Apicultura - 2023.xlsx
 * Total estimado: 39 registros
 * Programa no SIGMA: ID 42 - "Apicultura (Legado)"
 *
...
const ARQUIVO_PLANILHA = 'C:\\Users\\marce\\Downloads\\2023\\Apicultura - 2023.xlsx';
const PROGRAMA_ID = 42; // Apicultura (Legado)
const ANO_REFERENCIA = 2023;

interface RegistroApicultura {
  numero: number;
  produtor: string;
  linha: string;
  quantidade: number;
...
function extrairDadosAba(workbook: XLSX.WorkBook, sheetName: string): RegistroApicultura[] {
  const sheet = workbook.Sheets[sheetName];
  const dados = XLSX.utils.sheet_to_json(sheet, { header: 1, defval: '', raw: true }) as any[][];

  const registros: RegistroApicultura[] = [];
...
async function migrarApicultura() {
  console.log('='.repeat(80));
  console.log('MIGRAÇÃO - APICULTURA 2023');
  console.log('='.repeat(80));
  ...
  const todosRegistros: RegistroApicultura[] = [];
