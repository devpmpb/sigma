...
const validatePessoaCreate = (data: any) => {
  ...
  if (!data.tipoPessoa || !Object.values(TipoPessoa).includes(data.tipoPessoa)) {
    errors.push("Tipo de pessoa é obrigatório e deve ser válido (FISICA ou JURIDICA)");
  }
...
export const pessoaController = {
  ...
  create: async (req: Request, res: Response) => {
    try {
      const { tipoPessoa, pessoaFisica, pessoaJuridica, ...dadosPessoa } = req.body;
  ...
  update: async (req: Request, res: Response) => {
    try {
      ...
      if (dadosPessoa.tipoPessoa && dadosPessoa.tipoPessoa !== pessoaExistente.tipoPessoa) {
        return res.status(400).json({
