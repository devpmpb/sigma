...
const genericController = createGenericController({
  ...
  validateCreate: (data: any) => {
    ...
    if (!data.tipoEndereco || !Object.values(TipoEndereco).includes(data.tipoEndereco)) {
      errors.push("Tipo de endereço é obrigatório e deve ser válido");
    }
    
    // Validações para endereço urbano
    if (data.tipoEndereco === 'RESIDENCIAL' || data.tipoEndereco === 'COMERCIAL' || data.tipoEndereco === 'CORRESPONDENCIA') {
