...
import { authenticateToken, requirePermission, requireModuleAccess } from "../../middleware/authMiddleware";
...
// Rotas básicas CRUD
router.get("/", requirePermission(ModuloSistema.AGRICULTURA, AcaoPermissao.VIEW), arrendamentoController.findAll);
//router.get("/busca", 
//  requirePermission(ModuloSistema.AGRICULTURA, AcaoPermissao.VIEW), 
//  arrendamentoController.buscarPorTermo
//);
router.get("/:id", requirePermission(ModuloSistema.AGRICULTURA, AcaoPermissao.VIEW), arrendamentoController.findById);
router.post("/", requirePermission(ModuloSistema.AGRICULTURA, AcaoPermissao.CREATE), arrendamentoController.create);
router.put("/:id", requirePermission(ModuloSistema.AGRICULTURA, AcaoPermissao.EDIT), arrendamentoController.update);
router.delete("/:id", requirePermission(ModuloSistema.AGRICULTURA, AcaoPermissao.DELETE), arrendamentoController.delete);

// Rotas específicas do arrendamento
router.get("/proprietario/:proprietarioId",
  requirePermission(ModuloSistema.AGRICULTURA, AcaoPermissao.VIEW), 
  arrendamentoController.findByProprietario
);

router.get("/arrendatario/:arrendatarioId", 
  requirePermission(ModuloSistema.AGRICULTURA, AcaoPermissao.VIEW), 
  arrendamentoController.findByArrendatario
);

router.get("/propriedade/:propriedadeId", 
  requirePermission(ModuloSistema.AGRICULTURA, AcaoPermissao.VIEW), 
