...
import produtorService, { ProdutorDTO, Produtor, AreaEfetivaDTO } from "../../../../services/common/produtorService";
...
const ProdutorForm: React.FC<ProdutorFormProps> = ({ id, onSave }) => {
  ...
  useEffect(() => {
    const fetchPessoasFisicas = async () => {
      ...
      try {
        const pessoasFisicas = await pessoaService.getPessoasByTipo(TipoPessoa.FISICA);
  ...
  useEffect(() => {
    const loadProdutorData = async () => {
      if (id && id !== "novo") {
        try {
          const produtor = await produtorService.getProdutorWithDetails(Number(id));
          ...
          if (produtor.areaEfetiva) {
            setAreaEfetiva({
              ...
              areaArrendadaRecebida: Number(produtor.areaEfetiva.areaArrendadaRecebida),
              areaArrendadaCedida: Number(produtor.areaEfetiva.areaArrendadaCedida),
  ...
  }, [areaEfetiva.areaPropria, areaEfetiva.areaArrendadaRecebida, areaEfetiva.areaArrendadaCedida]);
  ...
  const validate = (values: ProdutorDTO): Record<string, string> | null => {
    ...
    if (incluirAreaEfetiva) {
      ...
      if (Number(areaEfetiva.areaArrendadaRecebida) < 0) {
        errors.areaArrendadaRecebida = "Área arrendada recebida não pode ser negativa";
      }

      if (Number(areaEfetiva.areaArrendadaCedida) < 0) {
        errors.areaArrendadaCedida = "Área arrendada cedida não pode ser negativa";
      }
    }
  ...
  return (
    <FormBase<Produtor, ProdutorDTO>
      ...
      service={{
        ...produtorService,
        create: (data: ProdutorDTO) => produtorService.create(buildFinalData(data)),
        ...
        <>
          ...
          {(!id || id === "novo") && (
            <div>
              ...
              >
                <option value="0">
                  {loadingPessoas ? "Carregando..." : "Selecione a pessoa física"}
                </option>
              ...
              <p className="mt-1 text-sm text-gray-500">
                Selecione a pessoa física que será cadastrada como produtor rural
              </p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Tipo de Produtor */}
            <div>
              <label htmlFor="tipoProdutor" className="block text-sm font-medium text-gray-700">
                Tipo de Produtor
              </label>
              <select
                id="tipoProdutor"
            ...
            {/* Atividade Principal */}
            <div>
              <label htmlFor="atividadePrincipal" className="block text-sm font-medium text-gray-700">
                Atividade Principal
              </label>
              <select
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* DAP */}
            <div>
              <label htmlFor="dap" className="block text-sm font-medium text-gray-700">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <label className="flex items-center">
                <input
                  ...
                  className="h-4 w-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                />
                <span className="ml-2 text-sm text-gray-700">Definir área efetiva</span>
              </label>
            </div>
              ...
              <>
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                  <div className="text-sm text-blue-800">
                    <strong>Área Efetiva:</strong> É calculada automaticamente como:
                    <br />
                    <code className="bg-blue-100 px-2 py-1 rounded">
                      Área Própria + Área Arrendada Recebida - Área Arrendada Cedida
                    </code>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    {errors.anoReferencia && (
                      <p className="mt-1 text-sm text-red-600">{errors.anoReferencia}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    />
                    {errors.areaPropria && (
                      <p className="mt-1 text-sm text-red-600">{errors.areaPropria}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {errors.areaArrendadaRecebida && (
                      <p className="mt-1 text-sm text-red-600">{errors.areaArrendadaRecebida}</p>
                    )}
                  ...
                  <div>
                    ...
                    {errors.areaArrendadaCedida && (
                      <p className="mt-1 text-sm text-red-600">{errors.areaArrendadaCedida}</p>
                    )}
                    <p className="mt-1 text-xs text-gray-500">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={`${produtorService.formatarArea(areaEfetiva.areaEfetiva || 0)} alqueires`}
                      disabled
                      className="mt-1 block w-full rounded-md bg-gray-100 border-gray-300 text-gray-700 font-semibold"
                    />
                    <p className="mt-1 text-xs text-gray-500">
                      Área efetivamente utilizada pelo produtor para a atividade rural
                    </p>
                  </div>
                </div>
              </>
