...
const RelatoriosArrendamento: React.FC = () => {
  ...
  const { data: relatorioGeral, isLoading: loadingGeral } = useQuery({
    ...
    queryFn: () =>
      relatorioArrendamentoService.geral({
  ...
  const { data: relatorioPorPropriedade, isLoading: loadingPorPropriedade } =
    useQuery({
      queryKey: ["relatorio-arrendamento-propriedade", dataInicio, dataFim],
      queryFn: () =>
        relatorioArrendamentoService.porPropriedade({
          dataInicio: dataInicio || undefined,
          dataFim: dataFim || undefined,
        }),
  ...
  const { data: relatorioPorArrendatario, isLoading: loadingPorArrendatario } =
    useQuery({
      queryKey: ["relatorio-arrendamento-arrendatario", dataInicio, dataFim],
      queryFn: () =>
        relatorioArrendamentoService.porArrendatario({
          dataInicio: dataInicio || undefined,
          dataFim: dataFim || undefined,
        }),
  ...
  const { data: relatorioPorAtividade, isLoading: loadingPorAtividade } =
    useQuery({
      queryKey: ["relatorio-arrendamento-atividade", dataInicio, dataFim],
      queryFn: () =>
        relatorioArrendamentoService.porAtividadeProdutiva({
          dataInicio: dataInicio || undefined,
          dataFim: dataFim || undefined,
        }),
  ...
  const { data: relatorioVencendo, isLoading: loadingVencendo } = useQuery({
    ...
    queryFn: () =>
      relatorioArrendamentoService.vencendo({
  ...
  const isLoading =
    loadingGeral ||
    loadingPorPropriedade ||
    loadingPorArrendatario ||
    loadingPorAtividade ||
    loadingVencendo;
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-900">
                {formatArea(data.estatisticas.areaTotal)}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-purple-900">
                {data.estatisticas.propriedadesUnicas}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-orange-900">
                {data.estatisticas.arrendatariosUnicos}
              </p>
            </div>
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-900">
                {data.totais.totalArrendamentos}
              </p>
            </div>
            ...
            <div className="flex justify-between items-start mb-3">
              ...
              <div className="text-right">
                ...
                <p className="text-xl font-bold text-gray-900">
                  {formatArea(item.propriedade.area)}
                </p>
              </div>
            </div>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {item.arrendamentos.map((arr, idx) => (
                      <tr key={idx}>
                        ...
                        <td className="px-4 py-2">
                          {formatDate(arr.dataInicio)} -{" "}
                          {formatDate(arr.dataFim)}
                        </td>
                        <td className="px-4 py-2">
        ...
        <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-purple-900">
                {data.totais.totalArrendatarios}
              </p>
            </div>
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-green-600 font-medium">Área Total</p>
              <p className="text-2xl font-bold text-green-900">
                {formatArea(data.totais.areaTotal)}
              </p>
            </div>
            <BarChart3 className="h-8 w-8 text-green-400" />
          </div>
          ...
          >
            <div className="flex justify-between items-start mb-3">
              <div>
                ...
                <p className="text-sm text-gray-600">
                  CPF/CNPJ: {item.arrendatario.cpfCnpj}
                </p>
                {item.arrendatario.telefone && (
                  <p className="text-sm text-gray-600">
                    Telefone: {item.arrendatario.telefone}
                  </p>
                )}
              </div>
              <div className="text-right">
            ...
            <div className="border-t border-gray-200 pt-3 mt-3">
              <div className="mb-3">
                <p className="text-sm text-gray-600">
                  Quantidade de Arrendamentos:{" "}
                  <strong>{item.quantidadeArrendamentos}</strong>
                </p>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {data.atividades.map((item, index) => (
              <tr key={index} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm text-gray-900">
                    {item.arrendatariosUnicos}
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-sm text-gray-900">
                    {item.propriedadesUnicas}
                  </div>
                </td>
              </tr>
            ))}
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {data.arrendamentos.map((item: any, index: number) => {
              ...
              return (
                <tr
                  key={index}
                  className={`hover:bg-gray-50 ${isUrgente ? "bg-red-50" : ""}`}
                >
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">
                      {item.arrendatario?.nome || "N/A"}
      ...
      <div className="bg-white shadow-md rounded-lg p-6">
        ...
        {isLoading ? (
          ...
          <>
            {tipoRelatorio === "geral" &&
              relatorioGeral &&
              renderRelatorioGeral(relatorioGeral)}
            {tipoRelatorio === "porPropriedade" &&
              relatorioPorPropriedade &&
              renderRelatorioPorPropriedade(relatorioPorPropriedade)}
            {tipoRelatorio === "porArrendatario" &&
              relatorioPorArrendatario &&
              renderRelatorioPorArrendatario(relatorioPorArrendatario)}
            {tipoRelatorio === "porAtividade" &&
              relatorioPorAtividade &&
              renderRelatorioPorAtividade(relatorioPorAtividade)}
            {tipoRelatorio === "vencendo" &&
              relatorioVencendo &&
              renderRelatorioVencendo(relatorioVencendo)}
          </>
        )}
      </div>
    </div>
